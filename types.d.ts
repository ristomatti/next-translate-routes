...
export type TReRoutes = {
    redirects: Redirect[];
    rewrites: Rewrite[];
};
export type TRouteSegmentPaths<L extends TAnyLocale> = {
    default: string;
} & Partial<Record<L, string>>;
export type TRouteSegmentData<L extends TAnyLocale> = string | ({
    default?: string;
} & Partial<Record<L, string>>);
...
export type TFallbackLng = string | string[] | {
    [key: string]: string[];
};
...
export type NextConfigCompleteWithNTR = NextConfigComplete & {
    i18n: NTRI18NConfig;
    translateRoutes: NTRConfig;
};
declare global {
